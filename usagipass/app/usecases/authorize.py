...
def verify_admin(token: Annotated[str, Depends(oauth2_scheme)], session: Session = Depends(require_session)):
    try:
        payload = jwt.decode(token, jwt_secret, algorithms=["HS256"])
        if (user := session.get(User, payload["username"])) and user.privilege == Privilege.ADMIN:
            return user
        raise HTTPException(
