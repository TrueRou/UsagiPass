...
::: danger 警告
此方法高度依赖动手能力，除非你安装用户证书后无效，否则不推荐此方法！<br>
本教程不对你因使用此方法而对设备造成的任何损害负责！
:::

::: tip 注意
`KernelSU` 及相关分支，需要额外搭配元模块才能让模块正常工作，<br>
...
::: tip 使用 UsagiPass 更新查分器
UsagiPass 支持从公众号同步你的成绩并更新至 UsagiLab 通行证绑定的查分器，且在系统缺少证书的状态下仍可工作，假如你不想安装证书或证书已安装但无效，还可有限地使用 UsagiPass。在完成第二步[配置代理软件](#第二步-配置代理软件)后：

1. 启用代理

2. 发送 https://up.turou.fun 或 https://up.turou.fun/auth/login 至微信任意聊天并访问

3. 注册或登录你的 UsagiLab 通行证（此步为必需，请不要使用访客模式）

4. 在设置页面按照提示绑定你的查分器账号，已有绑定可跳过

5. 回到主页，点击卡片左下角的小火箭图标，等待系统提示即可更新数据

请注意：此方式下无法使用二维码美化功能，且若未安装证书那么在代理启用下二维码页面也将无法正常打开。
:::
...
**Q：我不清楚 UsagiPass 的代理具体如何配置**

A：可以在左侧的目录（手机用户请点击左上角 Menu 按钮）中找到受支持代理的详细介绍。如果仍有疑问，可以[加群](https://qm.qq.com/q/c3l0WgWeWI)进行询问。
...
**Q：在按照步骤配置后，我无法打开 UsagiPass 网页或更新公众号数据出现异常**

A：UsagiPass 目前使用腾讯云托管，域名使用阿里云个人版云解析，并且已经在工信部备案，可保证国内各地区的正常访问。

若你在自行诊断后仍然出现无法连接的问题，请[加群](https://qm.qq.com/q/c3l0WgWeWI)联系我们，我们会根据你的地区向服务商发起工单，尝试解决你的问题。
