...
export const useDraftStore = defineStore('draft', () => {
    ...
    async function fetchPreferences(uuid?: string): Promise<Preference> {
        try {
            const path = uuid ? `/cards/${uuid}/preference` : '/preferences/defaults';
    ...
    async function testPreferences(preferences: Preference): Promise<boolean> {
        try {
            await userStore.axiosInstance.post("/preferences/test", preferences);
            return true;
        } catch (error: any) {
            notificationStore.error("存在不支持的字符", error.response.data.detail || "未知错误");
            return false;
        }
    }
    ...
    return { fetchDrafts, createDraft, fetchPreferences, patchPreferences, deleteDraft, testPreferences }
})